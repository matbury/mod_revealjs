 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_revealjs_generator extends testing_module_generator {

    /**
     * Create new revealjs module instance
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->name)) {
            $record->name = get_string('pluginname', 'revealjs').' '.$i;
        }
        if (!isset($record->intro)) {
            $record->intro = 'Test revealjs '.$i;
        }
        if (!isset($record->introformat)) {
            $record->introformat = FORMAT_MOODLE;
        }
        if (!isset($record->content)) {
            $record->content = 'Test revealjs content';
        }
        ...
        $id = revealjs_add_instance($record, null);
